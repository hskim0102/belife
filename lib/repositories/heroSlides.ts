...
const COLUMNS = `id, image_url, alt, sort_order, published, blob_pathname, created_at, updated_at`
...
export async function createHeroSlide(input: NewHeroSlide): Promise<HeroSlide> {
  const row = await queryOne<HeroSlideRow>(
    `INSERT INTO hero_slides (image_url, alt, blob_pathname, sort_order)
       VALUES ($1, $2, $3, COALESCE((SELECT MAX(sort_order) + 1 FROM hero_slides), 0))
       RETURNING ${COLUMNS}`,
    [input.imageUrl, input.alt, input.blobPathname],
  )
